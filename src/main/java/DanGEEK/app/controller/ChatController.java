...
public class ChatController {
    ...
    public void enter(@DestinationVariable(value = "chatRoomId") Long chatRoomId) {
        // 현재 유저의 입장 메시지 채팅 전송, DB에 저장
        chatService.enterChatRoom(chatRoomId);
        // 현재 유저와 채팅방 사이의 관계 매핑
        chatRoomMemberService.enterChatRoomMember(chatRoomId, SecurityUtil.getCurrentMemberId());
    }
    @MessageMapping("/chatroom/exit/{chatRoomId}")
    public void exit(@DestinationVariable(value = "chatRoomId") Long chatRoomId) {
        // 현재 유저의 퇴장 메시지 채팅 전송, DB에 저장
        chatService.exitChatRoom(chatRoomId);
        // 현재 유저와 채팅방 사이의 관계 삭제 및 엔티티가 존재하지 않을 시에 테이블 삭제
        chatRoomMemberService.deleteChatRoomMember(chatRoomId, SecurityUtil.getCurrentMemberId());
    }
